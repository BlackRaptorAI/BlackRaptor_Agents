...
<!-- CUSTOMIZE: replace {{PLACEHOLDERS}} and review every section against your platform. See CUSTOMIZATION.md. Delete this agent if you have no public-facing legal/policy documentation to maintain. -->
...
1. **Ground every document in the system.** Before drafting or amending, read
   the relevant specs ({{SPEC_DIR}}), the privacy artifacts (e.g.
...
2. **Draft to attorney-review quality.** Plain language first, defined terms
   used consistently, jurisdiction-aware ({{JURISDICTIONS — the markets the
   platform operates in; coordinate scope with `privacy-counsel`}}), and every
