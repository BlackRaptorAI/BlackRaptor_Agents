...
Target: `main` on `{{ORG}}/{{REPO}}`.
Context: written for a solo coder ({{DEPLOY_MODEL}});
...
- [ ] Create team **{{SECOND_KEY_TEAM}}**, add **your second approver only**, grant
...
- [ ] **Required approvals: 0.** Not a typo. With 1+, every PR needs an
      ...
      {{SECOND_APPROVER}}-gating comes from the next toggle, not this number.
- [ ] **Require review from Code Owners: ON.** With the sparse CODEOWNERS,
      only Tier-3 paths request {{SECOND_APPROVER}}; everything else merges without review.
      ...
      a Tier-3 path should be blocked until {{SECOND_APPROVER}} approves.
...
- [ ] **Dismiss stale pull request approvals when new commits are pushed: ON.**
      Otherwise you could get {{SECOND_APPROVER}}'s approval, then push different code and
...
- [ ] **Require conversation resolution before merging: ON.** Cheap; makes
      {{SECOND_APPROVER}}'s questions un-ignorable.
...
- Emergency path (prod down, {{SECOND_APPROVER}} unreachable): temporarily disable the rule
...
- [ ] Test PR touching a Tier-3 path → blocked until {{SECOND_APPROVER}} approves; approval
